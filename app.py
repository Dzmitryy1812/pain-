...
st.caption(
f"IV used: {iv_used100:.1f}% | ATM IV: {atm_iv100:.1f}% | Manual IV: {user_iv*100:.1f}% | r: {r_pct:.2f}%"
)
...
c1.metric("P(выше low)", f"{prob_above_low100:.1f}%")
c2.metric("P(ниже high)", f"{prob_below_high100:.1f}%")
...
value=f"Модель: {prob_above_low100:.1f}%",
delta=f"Edge: {edge_l100:+.1f}%",
)
if edge_l > 0.03:
st.success(f"✅ Выгодно покупать YES (edge {edge_l100:.1f}%)")
else:
st.error(f"❌ Дорого — нет преимущества (edge {edge_l100:.1f}%)")

with col2:
st.metric(
label=f"Верхний барьер {int_to_k(p_high_strike)} (${p_high_strike:,})",
value=f"Модель: {prob_below_high100:.1f}%",
delta=f"Edge: {edge_h100:+.1f}%",
)
if edge_h > 0.03:
st.success(f"✅ Выгодно покупать NO (edge {edge_h100:.1f}%)")
else:
st.error(f"❌ Дорого — нет преимущества (edge {edge_h100:.1f}%)")
...
Текущий Spot BTC: ${final_spot:,.0f}
...
0
:
Используйпредоставленныйдиапазон
(
c 
m
​
 ax:,.0f(Дистанция:high 
d
​
 ist 
p
​
 ct:.2fЗАДАЧА№0:Используйпредоставленныйдиапазон({c_min:,.0f} – ${c_max:,.0f}) как фактический. НЕ придумывай данные от себя.
...
Нижний барьер: {int_to_k(p_low_strike)} (
p
l
o
w
...
s
​
 trike:,.0f).Мояценазахода(YES):{p_low_price:.2f}. Себестоимость (BSM): {prob_above_low*100:.1f}%
Верхний барьер: {int_to_k(p_high_strike)} (
p
h
i
g
...
s
​
 trike:,.0f).Мояценазахода(NO):{p_high_price:.2f}. Себестоимость (BSM): {prob_below_high*100:.1f}%
...
Вердикт ИИ: ОДОБРЕНО / ПРОПУСК / ОПАСНО (и оценка 1-10). Кратко логику.
Анализ экстремумов:
Учитывая исторический 10d диапазон, находятся ли мои барьеры 
p
l
o
w
...
s
​
 trike:,.0fи{p_high_strike:,.0f} ВНУТРИ этого диапазона или они защищены им (ЗА пределами)?
Анализ RV vs IV (VRP):
...
Цели (Max Pain): Как движется Max Pain к {selected_exp}? Есть ли риск, что он притянет цену и пробьет барьер?
Макро-риск: Есть ли в календаре события на этих датах, способные вызвать скачок RV?
Финансовый Edge: Оправдывает ли общая стоимость входа (${p_low_price + p_high_price:.2f}) математическую вероятность успеха?
Наихудший сценарий: С какой стороны (сверху или снизу) риск пробоя выше с учетом GEX и Дистанций до экстремумов?
Риск-менеджмент: Оцени адекватность стоп-лосса -10% от стоимости всей конструкции.
"""
*.txt
Plaintext
st.success("✅ Промпт готов (RV, IV и Премия за риск добавлены)")
 st.code(prompt_text, language="markdown")   КАК ТЕБЕ
